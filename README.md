...
- Основной домен сайта — `https://vershina49.ru`; относительный `base` сохраняет корректную работу GitHub Pages и локальной сборки.
...
- Домен закреплён файлом `public/CNAME`; канонические URL, Open Graph, structured data, robots и sitemap используют `vershina49.ru`.
